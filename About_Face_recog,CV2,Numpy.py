...
yeongtaek_image = face_recognition.load_image_file("examples/yeongtaek.jpeg") # 김영택 이미지 파일 불러오기
yeongtaek_face_encoding = face_recognition.face_encodings(yeongtaek_image)[0] # 얼굴 인코딩 추출

sanghun_image = face_recognition.load_image_file("examples/sanghun.jpeg") # 추상헌 이미지 파일 불러오기
sanghun_face_encoding = face_recognition.face_encodings(sanghun_image)[0] # 얼굴 인코딩 추출

beongkwon_image = face_recognition.load_image_file("examples/beongkwon.jpeg") # 최병권 이미지 파일 불러오기
beongkwon_face_encoding = face_recognition.face_encodings(beongkwon_image)[0] # 얼굴 인코딩 추출

# 인식된 얼굴 인코딩과 그에 해당하는 이름의 배열을 생성합니다.
known_face_encodings = [ # 인식된 얼굴 인코딩 리스트
    obama_face_encoding,
    biden_face_encoding,
    yeongtaek_face_encoding,
    sanghun_face_encoding,
    beongkwon_face_encoding
]
...
while True:
    ...
    if process_this_frame:
        ...
        for face_encoding in face_encodings:
            ...
            matches = face_recognition.compare_faces(known_face_encodings, face_encoding, tolerance=0.6) # 얼굴 일치 여부 확인
            name = "Unknown" # 일치하는 얼굴이 없으면 "Unknown"으로 설정

            # 새로운 얼굴과의 거리가 가장 작은 알려진 얼굴을 사용
            ...
            if face_distances[best_match_index] < 0.5: # 가장 작은 거리의 인덱스가 일치하는 경우
